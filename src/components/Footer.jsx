  ...
  social: [
    {
      ...
      icon: (props) => (
        <svg fill="#474745" viewBox="0 0 24 24" {...props}>
          <path
            fillRule="evenodd"
...
export default function Footer() {
  return (
    <footer className="bg-light_gray h-56">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 pt-[60px] lg:px-8">
          ...
          {navigation.main.map((item, i) => (
            <Link to={item.scroll} smooth={true} duration={500} key={i}>
              <div key={item.name} className="pb-6">
                <p
                  className="text-sm leading-6 text-dark_gray hover:text-medium_gray"
                >
                  {item.name}
                </p>
              </div>
        ...
        <div className="mt-5 flex justify-center space-x-10">
          {navigation.social.map((item) => (
            <a
              ...
              key={item.name}
              href={item.href}
              className="text-dark_gray hover:text-medium_gray cursor-pointer"
            >
        ...
        <p className="mt-5 text-center text-xs leading-5 text-dark_gray">
          &copy; 2023 Arion, Inc. All rights reserved.
        </p>
      </div>
    </footer>
  );
